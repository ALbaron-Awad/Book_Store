...
//***************  GLOBAL  VARIABLES  ***************/
struct Node_Comp *Head = NULL; // Head of the Lunked List
int page_no; // page number will be used in producing reports
...
void AddBook(void) {
    ...
    Prev = Curr = NULL; // to find the previous and current position, loop is used
    ...
    NewNode->next = Prev->next; // add somewhere in middle or at the end
    Prev->next = NewNode;
} // end of Add function
...
void DeleteBook() {
    ...
    if (Curr->ISBN != data) {
        cout << "Sorry, but the ISBN of this Book not found  in the list !" << endl;
    } else {
        Prev->next = Curr->next;
    }
}
    ...
    while (true) {
        ...
        switch (ch) {
            ...
            case 3:
                ...
                Heading();
                for (Curr = Head; Curr != NULL; Curr = Curr->next) {
                    if (Curr->category == Look_category) {

                        cout << "\tForm the " << Curr->category << endl;
...
void SaveList(void) {
    ofstream pFile;
    pFile.open("Booktest.dat", ios::binary);
    if (pFile == NULL) {
...
void LoadList() {
    ifstream piFile;
    piFile.open("Booktest.dat", ios::binary);
    if (piFile == NULL) {
        cout << "File not Exist \n";
        return;
    }
